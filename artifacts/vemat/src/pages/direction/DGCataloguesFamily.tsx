...
export default function DGCataloguesFamily() {
  const { lang } = useLang();
  const [, params] = useRoute<{ family: string }>("/direction/catalogues/groupe/:family");
      ...
      <DGGuard>
        <DGLayout>
          <div className="p-10 max-w-3xl">
            <Link href="/direction/catalogues" className="text-sm text-purple-600 hover:underline inline-flex items-center gap-1 mb-4">
    ...
    <DGGuard>
      <DGLayout>
        <div className="p-6 lg:p-10 max-w-[1400px] mx-auto">
          <Link
            href="/direction/catalogues"
            className="text-xs text-zinc-500 hover:text-purple-600 inline-flex items-center gap-1 mb-4 transition-colors"
          >
          ...
          {machines !== null && filtered.length === 0 && (
            <div className="bg-white rounded-2xl border border-zinc-200 p-10 text-center">
              ...
              <p className="font-bold text-zinc-700 mb-1">
                {lang === "fr"
                  ? `Aucune machine ${meta.key} pour l'instant`
                  : `No ${meta.key} machines yet`}
              </p>
              <p className="text-sm text-zinc-500">
                {lang === "fr"
                  ? "Les catalogues apparaîtront ici dès qu'ils seront scrapés."
                  : "Catalogues will appear here once scraped."}
              </p>
            </div>
          )}
